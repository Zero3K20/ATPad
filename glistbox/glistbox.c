...
static LRESULT AddGListEntry(HWND hList, PGLIST_IMAGE pi, LPCSTR lpString);
...
/*-@@+@@--------------------------------[Do not edit manually]------------
 Procedure: CreateGListWindow
 Created  : Mon Jul 16 13:14:37 2007
 Modified : Mon Jul 16 13:24:19 2007

 Synopsys : Creates glist window
 Input    : dwExStyle - extended window style
            dwStyle - window style
            x - the initial horizontal position of the window
            y - the initial vertical position of the window
...
HWND CreateGListWindow(DWORD dwExStyle, DWORD dwStyle, int x, int y, int w, int h, HWND hParent){
	HWND			hList = NULL;
	WNDPROC			hProc, hListProc;
	...
	dwStyle |= (LBS_HASSTRINGS | LBS_NOINTEGRALHEIGHT | LBS_OWNERDRAWVARIABLE | LBS_NOTIFY | WS_VSCROLL);
	// create listbox window
	hList = CreateWindowEx(dwExStyle, "listbox", NULL, dwStyle, x, y, w, h, hParent, NULL, (HINSTANCE)GetWindowLongPtr(hParent, GWLP_HINSTANCE), NULL);
	if(hList == NULL)
		return NULL;
	// get listbox window procedure address and subclass it (must clear errors before call to SetWindowLongPtrW)
	SetLastError(0);
	hListProc = (WNDPROC)SetWindowLongPtr(hList, GWLP_WNDPROC, (LONG_PTR)ListProc);
	if(hListProc == 0 && GetLastError() != 0)
		return NULL;
	// store listbox window procedure address
	if(SetProp(hList, LST_PROC_PROP, hListProc) == 0)
		return NULL;
	// add parent window data to inner collection
	if(!GLStructExists(hParent)){
		// get parent window procedure address and subclass it (must clear errors before call to SetWindowLongPtrW)
		SetLastError(0);
		hProc = (WNDPROC)SetWindowLongPtr(hParent, GWLP_WNDPROC, (LONG_PTR)ParentProc);
		if(hProc == 0 && GetLastError() != 0)
			return NULL;
		// add parent handle and proc address to inner collection
		if(AddNewGLStruct(hParent, hProc) == NULL)
...
/*-@@+@@--------------------------------[Do not edit manually]------------
 Procedure: AddGListEntry
 Created  : Mon Jul 16 13:41:19 2007
 Modified : Mon Jul 16 13:41:19 2007

 Synopsys : Adds new entry to glist window
 Input    : hList - glist window handle
            lpString - item text
            pi - pointer to GLIST_IMAGE structure containing image/color 
            information
...
static LRESULT AddGListEntry(HWND hList, PGLIST_IMAGE pi, LPCSTR lpString){
	...
	if(pi->nType < GLIST_I_COLOR || pi->nType > GLIST_I_ICON)
		return -1;
	// add string to listbox
	res = SendMessage(hList, LB_ADDSTRING, 0, (LPARAM)lpString);
	if(res == LB_ERR || res == LB_ERRSPACE)
		return -1;
	// allocate memory for image/color infomation
	pData = (PGLIST_IMAGE)calloc(1, sizeof(GLIST_IMAGE));
	...
	memcpy(pData, pi, sizeof(GLIST_IMAGE));
	// store pointer to image/color information as item data
	if(SendMessage(hList, LB_SETITEMDATA, res, (LPARAM)pData) == LB_ERR)
		return -1;
	// if individual item height is specified
	if(pi->itemHeight > 0)
		// set listbox item height
		if(SendMessage(hList, LB_SETITEMHEIGHT, res, pi->itemHeight) == LB_ERR)
			return -1;
	return res;
}
...
static LRESULT CALLBACK ListProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam){
	...
	if(unicode)
		...
		hProc = (WNDPROC)GetProp(hwnd, LST_PROC_PROP);
	...
	case GLM_ADDENTRY:
		if(unicode)
			...
			return AddGListEntry(hwnd, (PGLIST_IMAGE)wParam, (LPCSTR)lParam);
	default:
...
static void GListOnDrawItem(LPDRAWITEMSTRUCT lpd){
	...
	// check Unicode flag
	unicode = IsWindowUnicode(lpd->hwndItem);
	...
	if(unicode)
		...
		pData = (PGLIST_IMAGE)SendMessage(lpd->hwndItem, LB_GETITEMDATA, lpd->itemID, 0);
	...
	if(unicode){
		...
		SendMessage(lpd->hwndItem, LB_GETTEXT, lpd->itemID, (LPARAM)szBuffer);
		DrawTextEx(lpd->hDC, szBuffer, -1, &rc, DT_LEFT | DT_SINGLELINE | DT_VCENTER, NULL);
	}
