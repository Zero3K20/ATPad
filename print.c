...
void PrintEdit(P_TPEDIT pE){
	...
	int				xNP, yNP;
	DOCINFOW		docinfo;
	wchar_t			szDocName[MAX_PATH], szPage[32];
	int				oldAlign;
	COLORREF		oldColor;
	...
	if(PrintDlgW(&g_pPdlg)){
		...
		if(pageMax){
			do{
				...
				SendMessageW(pE->hEdit, EM_FORMATRANGE, TRUE, (LPARAM)&fr);
				EndPage(hPrintDC);
			}while(pageMin < pageMax);
		}
		DeleteObject(hFontPageNumber);
		SendMessageW(pE->hEdit, EM_FORMATRANGE, FALSE, (LPARAM)NULL);
        EndDoc(hPrintDC);
        DeleteDC(hPrintDC);
	}
}
